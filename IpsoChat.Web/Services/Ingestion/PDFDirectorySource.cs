...
public class PDFDirectorySource(string sourceDirectory) : IIngestionSource
{
    ...
    private string GetSourceFileId(string path) => Path.GetRelativePath(sourceDirectory, path);
    ...
    public Task<IEnumerable<IngestedDocument>> GetNewOrModifiedDocumentsAsync(IReadOnlyList<IngestedDocument> existingDocuments)
    {
        ...
        foreach (var sourceFile in sourceFiles)
        {
            ...
            var sourceFileId = GetSourceFileId(sourceFile);
    ...
    public Task<IEnumerable<IngestedDocument>> GetDeletedDocumentsAsync(IReadOnlyList<IngestedDocument> existingDocuments)
    {
        if (!Directory.Exists(sourceDirectory))
        {
            // Directory is missing → treat as all documents deleted
            return Task.FromResult<IEnumerable<IngestedDocument>>(existingDocuments);
        }
        var currentFiles = Directory.GetFiles(sourceDirectory, "*.pdf", SearchOption.AllDirectories);
        var currentFileIds = currentFiles.Select(GetSourceFileId).ToHashSet();
